...
async function tvl(api) {
  for (const vault of VAULTS) {
    ...
    const exchangeRateScale = await call(vault.contract_id, "get_share_price_scale", {})
    const extraDecimalScale = await call(vault.contract_id, "get_extra_decimal_scale", {})
    const exchangeRate = await call(vault.contract_id, "get_share_price_in_asset", { asset: vault.asset })
