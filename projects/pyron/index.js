...
const programId = 'PyRon8FBSDSk6MxNKsZj2uZweBsa2nH5amyKnN6eN57'

async function getBanks() {
  const provider = getProvider('fogo')
  const program = new Program(idl, programId, provider)
  return program.account.bank.all()
}

async function tvl() {
  const allBanks = await getBanks()
  return sumTokens2({ chain: 'fogo', tokenAccounts: allBanks.map(i => i.account.liquidityVault.toString()) });
}
...
const I80F48_DIVISOR = 2n ** 48n;

function wrappedI80F48toNumber(wrapped) {
  const val = BigInt(wrapped.value.toString())
  const integer = val / I80F48_DIVISOR
  const remainder = val % I80F48_DIVISOR
  if (remainder === 0n) return Number(integer)
  const decimal = (remainder * 1000000n) / I80F48_DIVISOR
  return Number(integer.toString() + '.' + decimal.toString().padStart(6, '0'))
}
