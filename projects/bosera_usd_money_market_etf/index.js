...
async function tvl(api) {
  const price = await api.call({ target: BSUMETU, abi: "int256:latestAnswer", });
  const priceDecimals = await api.call({ target: BSUMETU, abi: "uint8:decimals", });
  const supply = await api.call({ target: BoseraUSDMMF, abi: 'erc20:totalSupply', });
  const tokenDecimals = await api.call({ target: BoseraUSDMMF, abi: 'erc20:decimals', });
